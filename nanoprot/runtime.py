"""
Runtime utilities for nanoprot: device + dtype detection, DDP init, logging.
"""
...
# Override with NANOPROT_DTYPE env var: "bfloat16", "float16", "float32"
_DTYPE_MAP = {"bfloat16": torch.bfloat16, "float16": torch.float16, "float32": torch.float32}
def _detect_compute_dtype():
    env = os.environ.get("NANOPROT_DTYPE")
    if env is not None:
        return _DTYPE_MAP[env], f"set via NANOPROT_DTYPE={env}"
    if torch.cuda.is_available():
        ...
        # Users can still force fp16 via NANOPROT_DTYPE=float16 if they know what they're doing.
...
def get_base_dir():
    # co-locate nanoprot intermediates with other cached data in ~/.cache (by default)
    if os.environ.get("NANOPROT_BASE_DIR"):
        base_dir = os.environ.get("NANOPROT_BASE_DIR")
    else:
        home_dir = os.path.expanduser("~")
        cache_dir = os.path.join(home_dir, ".cache")
        base_dir = os.path.join(cache_dir, "nanoprot")
